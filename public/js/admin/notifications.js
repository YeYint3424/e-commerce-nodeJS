...
async function resolveNotificationUrl(notification) {
  if (notification.entityType === 'Order' && notification.entityId) {
    return `/ecommerce-admin/orders?orderId=${notification.entityId}`;
  }
  if (notification.entityType === 'Payment' && notification.entityId) {
    ...
    return orderId ? `/ecommerce-admin/orders?orderId=${orderId}` : null;
  }
  return null;
}
...
export function initNotificationBell(user) {
    ...
    if (!notifications.length) {
      ...
      panel.innerHTML = `
        <div class="flex items-center justify-between px-2 pb-2">
          ...
          <button type="button" id="admin-notif-mark-all" class="text-xs font-semibold text-gold-600 hover:underline dark:text-gold-400">Mark all read</button>
        </div>
        <div class="space-y-1">${notifications.map(notificationItemHtml).join('')}</div>
      `;
      panel.querySelector('#admin-notif-mark-all').addEventListener('click', async () => {
        try {
